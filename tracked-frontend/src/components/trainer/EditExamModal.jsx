...
const EditExamModal = ({ isOpen, onClose, onSuccess, examData, batches = [] }) => {
  ...
  const [questions, setQuestions] = useState([
    {
      id: null, // for existing questions
      question: '',
  ...
  useEffect(() => {
    if (isOpen && examData) {
      setFormData({
        ...
        duration: examData.time_limit || 60,
        passing_score: examData.passing_score || 85,
        retake_limit: examData.retake_limit || 1,
      ...
      if (examData.questions && examData.questions.length > 0) {
        setQuestions(examData.questions.map(q => ({
          ...
          options: q.options && q.options.length > 0 
            ...
            : q.type === 'true_false'
            ? [
                { id: null, option_text: 'True', is_correct: false },
  ...
  const addOption = (questionIndex) => {
    const newQuestions = [...questions];
    newQuestions[questionIndex].options.push({ id: null, option_text: '', is_correct: false });
    setQuestions(newQuestions);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onSuccess(examData.id, quizData);
      onClose();
    } catch (error) {
      console.error('Error updating exam:', error);
      setErrors({ submit: error.message || 'Failed to update exam' });
    } finally {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            <h2 className="text-2xl font-semibold text-gray-900">Edit Written Exam</h2>
            <p className="text-sm text-gray-600 mt-1">Update exam details and questions</p>
          </div>
          <button
            onClick={onClose}
